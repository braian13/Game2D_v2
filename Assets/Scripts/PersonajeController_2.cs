...
//            animator.SetFloat("VelocidadY", rb.velocity.y);
            ...
            //Salto
            if (Input.GetKey(KeyCode.W) && enElsuelo)
            {
        ...
        //Disparar
        if(Input.GetKey(KeyCode.E) && Time.time>lastShoot+0.5f && enElsuelo){
            Shoot();
            lastShoot = Time.time;
        }
        ...
        private void Shoot(){
            ...
            GameObject bullet =Instantiate(BulletPrefat, transform.position + direccionBala* 0.1f, Quaternion.identity);
